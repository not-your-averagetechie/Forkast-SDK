...
// Add missing MARKET_LIST_API_URL assignment for mainnet/testnet
const MARKET_LIST_API_URL =
    NETWORK === 'mainnet'
        ? process.env.MAINNET_MARKET_LIST_API_URL || 'https://api.forkast.gg/api/v1/markets'
        : process.env.TESTNET_MARKET_LIST_API_URL || 'http://localhost:3000/markets';
...
// Utility to print which wallet/account is being used for each order
function printOrderAccountInfo(orderBody: any, orderType: 'YES' | 'NO') {
    console.log(`Placing ${orderType} order with wallet: ${orderBody.account.wallet}`);
    console.log(`Proxy wallet: ${orderBody.account.proxy_wallet}`);
    console.log(`Signer: ${orderBody.account.private_key ? 'Private key present' : 'No private key'}`);
}

// Update order placement to show wallet info before placing
async function placeOrderWithRetry(orderBody: any, expenseLogger: ExpenseLogger, orderType: 'YES' | 'NO', cost: number, marketId: number, price: number, amount: number) {
    let attempts = 0;
    printOrderAccountInfo(orderBody, orderType); // Show wallet info
    while (attempts < 3) {
        try {
            await placeOrder(orderBody);
            expenseLogger.logSuccessfulOrder(orderType, cost);
            console.log(`✅ ${orderType} order placed for Market ${marketId} at $${price} (${amount} shares) - Cost: $${cost.toFixed(2)}`);
            return true;
        } catch (e) {
            attempts++;
            console.error(`❌ Failed ${orderType} order for Market ${marketId} at $${price}: ${e.message}`);
            if (e.message && e.message.toLowerCase().includes('insufficient funds')) {
                console.log('\n🚨 Insufficient funds detected. Please fund your account and press ENTER to resume...');
    ...
    console.error(`❌ Order for Market ${marketId} at $${price} (${amount} shares) failed after 3 attempts. Will retry after all other orders.`);
    return false;
}
...
async function fetchActiveMarkets(): Promise<any[]> {
    ...
    try {
        const eventsResponse = await axios.get(EVENT_API_URL, { params: { status: 'active', limit: 10 } });
        // Some APIs return { events: [...] }, some just { markets: [...] }
        if (eventsResponse.data && Array.isArray(eventsResponse.data.events)) {
            // Flatten all markets from all events
            return eventsResponse.data.events.flatMap((ev: any) => Array.isArray(ev.markets) ? ev.markets : []);
        }
        if (eventsResponse.data && Array.isArray(eventsResponse.data.markets)) {
            return eventsResponse.data.markets;
        }
    } catch (e) {
        // Ignore
    }
    ...
    for (const { marketId, startDigit } of marketConfigs) {
        try {
            ...
            for (const order of yesOrders) {
                ...
                const cost = order.price * order.amount;
                const success = await placeOrderWithRetry(orderBody, expenseLogger, 'YES', cost, market.id, order.price, order.amount);
                if (!success) {
                    failedYesOrders.push({ orderBody, orderType: 'YES', cost, marketId: market.id, price: order.price, amount: order.amount });
                }
            ...
            for (const order of noOrders) {
                ...
                const cost = order.price * order.amount;
                const success = await placeOrderWithRetry(orderBody, expenseLogger, 'NO', cost, market.id, order.price, order.amount);
                if (!success) {
                    failedNoOrders.push({ orderBody, orderType: 'NO', cost, marketId: market.id, price: order.price, amount: order.amount });
                }
                await new Promise(res => setTimeout(res, 1000));
            }

            // Retry failed orders after all others
            if (failedYesOrders.length > 0 || failedNoOrders.length > 0) {
                console.log('\n🔄 Retrying failed orders...');
            }
            for (const failed of failedYesOrders.concat(failedNoOrders)) {
                await placeOrderWithRetry(failed.orderBody, expenseLogger, failed.orderType, failed.cost, failed.marketId, failed.price, failed.amount);
            }
